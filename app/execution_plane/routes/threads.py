...
async def chat_with_thread(
    ...
    try:
        # 1. Creiamo la nuova chat nel db se non esiste, tramite thread_id
        thread_service.create_thread_if_not_exists(
            db,
            ChatThreadCreate(thread_id=thread_id, title=f"Chat {thread_id[:8]}")
        )

        # 1. Configura la memoria per questa conversazione specifica. Usiamo il context manager per ottenere il checkpointer asincrono
        ...
        # 3. Verifica team esistente
        db_team = team_db.get_team_by_id(db=db, team_id=payload.team_id)
        if db_team is None:
            raise HTTPException(status_code=404, detail=f"Team con ID {payload.team_id} non trovato. Impossibile avviare la chat.")

        # 4. Costruisci il grafo per questa richiesta
        try:
            graph = build_team_graph(db=db, team_id=payload.team_id, checkpointer=checkpointer)
...
def delete_agent(thread_id: str,
                 db: Session = Depends(get_db)):
    """
    Cancella l'agente con id specificato dal sistema
    """
    try:
        thread_service.delete_agent(db=db, thread_id=thread_id)
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))